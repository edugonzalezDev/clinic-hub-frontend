...
const Login = () => {
  ...
  const onLogin = async ({ email, password }: LoginForm) => {
    try {
      ...
      navigate(userType === "patient" ? "/paciente" : "/doctor", {
        replace: true,
      });
    } catch (err: unknown) {
  ...
  const onRegister = async (v: RegisterForm) => {
    ...
    if (v.date_of_bird > today)
      return toast.error("La fecha de nacimiento no puede ser futura.");
    if (v.date_of_bird.slice(0, 4) < "1900")
      return toast.error("La fecha de nacimiento no puede ser antes de 1900.");
        ...
        <CardContent>
          ...
          >
            ...
            <TabsContent value="register" className="space-y-4">
              <form
                onSubmit={handleSubmitReg(onRegister)}
                className="space-y-4"
                noValidate
              >
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  {errorsReg.sex && (
                    <p className="text-sm text-red-600">
                      {errorsReg.sex.message as string}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    {...registerReg("date_of_bird", {
                      required: "La fecha de nacimiento es obligatoria",
                    })}
                  />
                  {errorsReg.date_of_bird && (
                    <p className="text-sm text-red-600">
                      {errorsReg.date_of_bird.message as string}
                    </p>
                  )}
                </div>
